...
def train(
    ...
    for batch_idx, batch in enumerate(tqdm_meter):
        ...
        # targets = {L1: [batch size], ...}

        # compute loss
        losses = [criterion(logits[f'L{idx + 1}'], targets[f'L{idx + 1}']) for idx in range(len(targets))]
...
def evaluate(
    ...
    flattened_targets = torch.cat(
        [torch.cat([_t[f'L{idx + 1}'].view(-1, 1) for idx in range(len(_t))], dim=1) for _t in all_targets]).tolist()
