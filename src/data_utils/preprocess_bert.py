...
def preprocess_bert_data(train_df: pd.DataFrame, label_col_names: List[str], val_df: pd.DataFrame,
    ...
    # define label fields
    LABELS = [LabelField(['0', '1']) for _ in range(len(label_col_names))]

    # create datasets
    train_data = BertGermEvalDataset(df=train_df, label_col_names=label_col_names, bert_tokenizer=tokenizer,
                                     label_fields=LABELS)
    val_data = BertGermEvalDataset(df=val_df, label_col_names=label_col_names, bert_tokenizer=tokenizer,
                                   label_fields=LABELS)
    test_data = BertGermEvalDataset(df=test_df, label_col_names=label_col_names, bert_tokenizer=tokenizer,
    ...
    for idx in range(5):
        logger.info(
            ...
            f'label ids: {str([train_data[idx]["labels"][f"L{label_idx + 1}"].tolist() for label_idx in range(len(label_col_names))])}'
        )
