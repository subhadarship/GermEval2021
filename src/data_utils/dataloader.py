...
class SMARTTOKDataLoader(DataLoader):
    ...
    def __init__(self, dataset: Union[GermEvalDataset, BertGermEvalDataset], max_tokens: int, pad_idx: int,
                 shuffle: bool,
    ...
    def create_batches(self):
        ...
        bar = tqdm(self.order, desc='initialize epoch', unit=' batches',
                   leave=False) if self.progress_bar else self.order
        for idx in bar:
    ...
    def get_batch(self, pos):
        ...
        return {
            ...
            'labels': {
                f'L{idx + 1}': torch.cat([sample['labels'][f'L{idx + 1}'] for sample in samples]).to(self.device) for
                idx in range(len(self.dataset.label_col_names))
            }
        }
